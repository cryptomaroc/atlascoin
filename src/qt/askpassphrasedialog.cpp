    ...
    fCapsLock(false)
{
    ...
    switch(mode)
    {
        ...
        case Unlock: // Ask passphrase
            ...
            break;
        case UnlockTxn: // Ask passphrase, minting only disabled
            ui->warningLabel->setText(tr("This operation needs your wallet passphrase to unlock  the wallet."));
            ui->passLabel2->hide();
            ui->passEdit2->hide();
            ui->passLabel3->hide();
            ui->passEdit3->hide();
            ui->mintingCheckBox->setChecked(false);
            ui->mintingCheckBox->hide();
            setWindowTitle(tr("Unlock wallet for transaction"));
            break;
            ...
            if(newpass1 == newpass2)
            {
                if(model->setWalletEncrypted(true, newpass1))
                {
                    QMessageBox::warning(this, tr("Wallet encrypted"),
                                         "<qt>" + 
                                         tr("AtlasCoin will close now to finish the encryption process. "
