    ...
    rpcConsole(0)
{
  ...
  //connect(mainPage->sideBar, SIGNAL(clickedLogoImage()), this, SLOT(aboutClicked()));
    ...
    setWindowTitle(tr("AtlasCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
//    connect(mainPage->pageTransactions->table->table, SIGNAL(doubleClicked(QModelIndex)), this, mainPage->pageTransactions, SLOT(showDetails()));
    ...
    connect(mainPage->pageReceiveATC, SIGNAL(signMessage(QString)), this, SLOT(gotoSignMessageTab(QString)));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a AtlasCoin address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About AtlasCoin"), this);
    aboutAction->setToolTip(tr("Show information about AtlasCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for AtlasCoin"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("AtlasCoin client") + QString(" ") + tr("[testnet]"));
        ...
        mainPage->pageReceiveATC->setOptionsModel(clientModel->getOptionsModel());
    }
}
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("AtlasCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to AtlasCoin network.", "", count));
}
...
void BitcoinGUI::gotoReceiveCoinsPage()
{
    ...
    showSelectedPageItem(SC_PAGE_ID_RECEIVE_ATC);

    exportAction->setEnabled(true);
    disconnect(exportAction, SIGNAL(triggered()), 0, 0);
    connect(exportAction, SIGNAL(triggered()), mainPage->pageReceiveATC, SLOT(exportClicked()));
}

void BitcoinGUI::gotoSendCoinsPage()
{
    sendCoinsAction->setChecked(true);
    showSelectedPageItem(SC_PAGE_ID_SEND_ATC);
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        {
            if (mainPage->pageSendATC->sendCoinsPage->handleURI(uri.toString()))
                nValidUrisFound++;
        }

        // if valid URIs were found
        if (nValidUrisFound)
            slotSelectedPageItem(SC_PAGE_ID_SEND_ATC);
        else
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid AtlasCoin address or malformed URI parameters."));
    }
...
{
    // URI has to be valid
    if (mainPage->pageSendATC->sendCoinsPage->handleURI(strURI))
    {
...
void BitcoinGUI::slotSelectedPageItem(int id)
{
    ...
    case SC_PAGE_ID_SEND_ATC:
        showSelectedPageItem(page_id);
        exportAction->setEnabled(false);
        disconnect(exportAction, SIGNAL(triggered()), 0, 0);
        break;

    case SC_PAGE_ID_RECEIVE_ATC:
        showSelectedPageItem(page_id);
        exportAction->setEnabled(true);
        disconnect(exportAction, SIGNAL(triggered()), 0, 0);
        connect(exportAction, SIGNAL(triggered()), mainPage->pageReceiveATC, SLOT(exportClicked()));
