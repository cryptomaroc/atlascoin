...
// Cioyright (c) 2015-2017 The AtlasCoin developers
...
int64 nChainStartTime = 1478044882;
...
const string strMessageMagic = "AtlasCoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	nSubsidy = 40000000 * COIN; // Blocks 1-10 are premine
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in AtlasCoin
    bool fStrictPayToScriptHash = true; // Always active in AtlasCoin
...
if (!CheckCoinStakeTimestamp(GetBlockTime(), (int64)vtx[1].nTime))
    ...
    if ((nTime >= CUTOFF_POW_TIME) && (IsProofOfWork()))
         return DoS(100, error("CheckBlock() : Proof of work (%f ATC) at t=%d on or after %d.\n",
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AtlasCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "20161101 ATC proves that pzTimestamp is overkill.";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1478044882;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1809423; // perfect nonce
        ...
        assert(block.hashMerkleRoot == uint256("0x7c96a7e4620c78dde1f073b2a635a11e724b3956c73c18a46388efa7db9e9937"));
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running AtlascoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadAtlasCoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadAtlasCoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadAtlasCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d AtlasCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBitcoinMiner, pwallet))
                printf("Error: NewThread(ThreadAtlasCoinMiner) failed\n");
            Sleep(10);
        }
    }
}
